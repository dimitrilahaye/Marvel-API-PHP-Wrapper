...
class ApiComponent
{
	...
	* @throws ErrorException
	*/
	static public function post(string $url, array $body = array(), array $header = array())
	{
		try {
			return ApiComponent::_call("POST", $url, $body, $header);
		} catch (ErrorException $e) {
			throw $e;
		}
	}
	...
	* @throws ErrorException
	*/
	static public function put(string $url, array $body = array(), array $header = array())
	{
		try {
			return ApiComponent::_call("PUT", $url, $body, $header);
		} catch (ErrorException $e) {
			throw $e;
		}
	}
	...
	* @throws ErrorException
	*/
	static public function delete(string $url, array $body = array(), array $header = array())
	{
		try {
			return ApiComponent::_call("DELETE", $url, $body, $header);
		} catch (ErrorException $e) {
			throw $e;
		}
	}
	...
	*/
	static private function _call(string $method, string $url, array $body = array(), array $header = array())
	{
		try {
			...
			# set curl options depending of the method
			switch ($method) {
				case "POST":
			...
			# add body to request for all the methods except "GET"
			if (!in_array($method, array("GET")) && sizeof($body) > 0) {
